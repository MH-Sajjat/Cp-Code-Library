    ...
    void build(vector <vector< T>>& v) {
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            for (int l = 1; l <= logM; ++l) {
                for (int j = 1; j + (1 << l) - 1 <= m; ++j) {
                    spt[i][0][j][l] = max(spt[i][0][j][l - 1], spt[i][0][j + (1 << (l - 1))][l - 1]);
                }
            }
        }
        ...
        for (int k = 1; k <= logN; ++k) {
            for (int i = 1; i + (1 << k) - 1 <= n; ++i) {
                for (int l = 1; l <= logM; ++l) {
                    for (int j = 1; j + (1 << l) - 1 <= m; ++j) {
                        spt[i][k][j][l] = max(spt[i][k - 1][j][l], spt[i + (1 << (k - 1))][k - 1][j][l]);
                    }
                }
            }
        }
...
public:
    ...
    T query(int x1, int y1, int x2, int y2) {
        ...
        T mxR1 = max(spt[x1][logX][y1][logY], spt[x1][logX][y2 - (1 << logY) + 1][logY]);
        T mxR2 = max(spt[x2 - (1 << logX) + 1][logX][y1][logY], spt[x2 - (1 << logX) + 1][logX][y2 - (1 << logY) + 1][logY]);
        return max(mxR1, mxR2);
    }
};
